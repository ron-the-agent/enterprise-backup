...
public:
    ...
    BackupStats run(const fs::path& source, const fs::path& dest) {
        ...
        // Phase 3: Execute based on selected mode, consuming from the queue.
        switch (config_.mode) {
        ...
        // Wait for scanner to finish (it should already be done by now).
        scannerThread.join();
        ...
        LOG_INFO("Backup complete. Copied: " + std::to_string(stats.filesCopied) +
